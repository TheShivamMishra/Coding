...
Node* addDataBst(Node* root, int data)
{
    if (root == nullptr)
    {
    ...
    if (data < root->data)
        root->left = addDataBst(root->left, data);
    else
        root->right = addDataBst(root->right, data);

    return root;
}


Node* removeDataBst(Node* root, int data)
{
    if (data < root->data)
        root->left = removeDataBst(root, data);
    else if (data > root->data)
        root->right = removeDataBst(root, data);
    else
    {
